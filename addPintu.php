    ...
    if ( !empty($_POST)) { 
        ...
        if (!empty($gambar)){
            if (in_array($ekstensi, $ekstensi_diperbolehkan) === true){
                ...
                $_POST['gambar'] = $gambar;
                $file = file_get_contents('pintu.json');
                $data = json_decode($file, true);
                unset($_POST["add"]);
                $data["records"] = array_values($data["records"]);
                array_push($data["records"], $_POST);
                file_put_contents("pintu.json", json_encode($data));
                header("Location: pintu.php");
            }else{
                header("Location: addPintu.php?err=GX");
            }
        }else {
            header("Location: addPintu.php?err=GM");
        }          
    }
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Wayfinding - Pintu Masuk</title>
    <!-- CSS -->
                                      ...
                                      <div class="col-sm-5 col-sm-offset-3">
                                            ...
                                            <form method="POST" action="" enctype="multipart/form-data">
                                               <div class="form-group">
                                                <label for="namaRuangan">Nama Pintu Masuk</label>
                                                <input type="text" class="form-control" required="required" id="nama" name="nama" placeholder="Nama Pintu Masuk">
                                               ...
                                               <div class="form-actions">
                                                 ...
                                                 <a class="btn btn btn-danger" href="pintu.php">kembali</a>
                                               </div>
                                            </form>
                                    </div>
                                </div>
